...
class Game {
    ...
    Game(int id, String name, String releaseDate, int estimatedOwners, float price,
         String[] supportedLanguages, int supportedLanguagesCount, int metacriticScore, float userScore, int achievements,
...
public class HeapSortSemArrayList {
    ...
    public static void main(String[] args) {
        ...
        String entrada;
        entrada = sc.nextLine();
        // Criando um array dos ids de entrada
        String[] ids = new String[NoArrayList.MAX_IDS]; 
        int idsTamanho = 0; 

        // Capturando os ids de entrada e simulando o add() do ArrayList
        while (!entrada.equals("FIM") && idsTamanho < ids.length) {
            if (idsTamanho < ids.length) {
                ids[idsTamanho++] = entrada; // Adiciona e incrementa o contador
            }
            entrada = sc.nextLine();
        }

        // Criando o Array apenas com os jogos digitados
    ...
    // Adaptando a função para arrays, passando o tamanho como argumento
    static Game[] ordenacaoHeapSort(Game[] gameList, int tam) {

        Game[] tmp = new Game[tam + 1]; // Array temporário com índice 1 baseado
        ...
        Game[] tmp2 = new Game[tam]; // Array final indexado em 0
        // Copiando de volta para um array indexado em 0
        for (int i = 0; i < tam; i++) {
            tmp2[i] = gameList[i + 1];
        }
        gameList = tmp2;
    ...
    // Função swap adaptada para arrays 
        ...
        for (int i = 0; i < tamanho; i++) {
            ...
            System.out.println(
                    "=> " + jogosOrdenados[i].id + " ## " + jogosOrdenados[i].name + " ## " + releaseDate
                            ...
                            + " ## " + printArray(jogosOrdenados[i].supportedLanguages, jogosOrdenados[i].supportedLanguagesCount)
                            ...
                            + " ## " + (jogosOrdenados[i].tagsCount == 0 ? "" : printArray(jogosOrdenados[i].tags, jogosOrdenados[i].tagsCount))
...
class JogosDigitados {
    ...
    static Game[] inicializacao(String[] idArray, int tamanho) {
        ...
        while (sc.hasNextLine() && idsTamanho > 0 && gamesListTamanho < gamesList.length) {
            ...
            if (indiceEncontrado != -1) { 
                ...
                Game jogo = new Game(id, name, releaseDate, estimatedOwners, price,
                        ...
                        publishers, publishersCount, developers, developersCount, categories, categoriesCount, genres, genresCount, tags, tagsCount);
        ...
        // Redimensiona o array final para o tamanho real, mantendo a lógica do seu código original
