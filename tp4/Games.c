...
int main() {
    ...
    if (!arquivo) { perror("Erro ao abrir o arquivo"); return 1; }

    char buffer[TAM_LINHA];
    int totalJogos = 0;

    fgets(buffer, TAM_LINHA, arquivo); // pula cabeçalho
    while (fgets(buffer, TAM_LINHA, arquivo)) totalJogos++;
    fclose(arquivo);

    Game* jogos = (Game*) malloc(sizeof(Game) * totalJogos);
    if (!jogos) { printf("Falha na alocação de memória.\n"); return 1; }
    ...
    for (int k = 0; k < totalJogos; k++) liberarJogo(&jogos[k]);
    free(jogos);
    return 0;
}
...
char* lerCampo(char* linha, int* pos) {
    ...
    if (linha[*pos] == '"') { entreAspas = true; (*pos)++; }

    while (linha[*pos] && (entreAspas || linha[*pos] != ',')) {
        if (entreAspas && linha[*pos] == '"') { (*pos)++; break; }
        campo[i++] = linha[(*pos)++];
    }
...
char** dividirCampos(char* texto, char delimitador, int* qtd) {
    int separadores = 0;
    for (int i = 0; texto[i]; i++) if (texto[i] == delimitador) separadores++;
    *qtd = separadores + 1;
...
char* removerEspacos(char* texto) {
    ...
    while (fim > copia && isspace((unsigned char)*fim)) *fim-- = '\0';
...
char* ajustarData(char* entrada) {
    ...
    char* mesNum = "01";
    if (!strcmp(mesAbrev,"Feb")) mesNum="02";
    else if (!strcmp(mesAbrev,"Mar")) mesNum="03";
    else if (!strcmp(mesAbrev,"Apr")) mesNum="04";
    else if (!strcmp(mesAbrev,"May")) mesNum="05";
    else if (!strcmp(mesAbrev,"Jun")) mesNum="06";
    else if (!strcmp(mesAbrev,"Jul")) mesNum="07";
    else if (!strcmp(mesAbrev,"Aug")) mesNum="08";
    else if (!strcmp(mesAbrev,"Sep")) mesNum="09";
    else if (!strcmp(mesAbrev,"Oct")) mesNum="10";
    else if (!strcmp(mesAbrev,"Nov")) mesNum="11";
    else if (!strcmp(mesAbrev,"Dec")) mesNum="12";
...
void interpretarLinha(Game* j, char* linha) {
    ...
    if (linguas[0] == '[') memmove(linguas, linguas + 1, strlen(linguas));
    linguas[strcspn(linguas, "]")] = '\0';
    for (int i = 0; linguas[i]; i++) if (linguas[i] == '\'') linguas[i] = ' ';
...
void exibirJogo(Game* j) {
    ...
    printf(" ## %d ## %.1f ## %d ## ",
           ...
           j->achievements);
    exibirArray(j->publishers, j->publishersCount); printf(" ## ");
    exibirArray(j->developers, j->developersCount); printf(" ## ");
    exibirArray(j->categories, j->categoriesCount); printf(" ## ");
    exibirArray(j->genres, j->genresCount); printf(" ## ");
    exibirArray(j->tags, j->tagsCount); printf(" ##\n");
}

void liberarJogo(Game* j) {
    free(j->name);
    free(j->releaseDate);
    for (int i = 0; i < j->supportedLanguagesCount; i++) free(j->supportedLanguages[i]);
    free(j->supportedLanguages);
    for (int i = 0; i < j->publishersCount; i++) free(j->publishers[i]);
    free(j->publishers);
    for (int i = 0; i < j->developersCount; i++) free(j->developers[i]);
    free(j->developers);
    for (int i = 0; i < j->categoriesCount; i++) free(j->categories[i]);
    free(j->categories);
    for (int i = 0; i < j->genresCount; i++) free(j->genres[i]);
    free(j->genres);
    for (int i = 0; i < j->tagsCount; i++) free(j->tags[i]);
    free(j->tags);
}
