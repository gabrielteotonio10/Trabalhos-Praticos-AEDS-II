...
class Fila {
    ...
    public void enfileirar(GameAS game) {
        lista.inserirFim(game);
    }

    // Desenfileirar (remover do início)
    public GameAS desenfileirar() throws Exception {
        if (lista.tamanho() == 0) {
...
public class FilaJogos {
    ...
    public static void main(String[] args) {
        ...
        // Capturando os jogos digitados em uma fila
        Fila fila = new Fila();
        inicializarFilaComJogos(fila, ids, idsTamanho);
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (operacao) {
                ...
                case "R":
                    try {
                        GameAS removido = fila.desenfileirar();
        ...
        // Mostrando elementos restantes na fila
        fila.mostrar();
    }

    // Método para inicializar a fila com jogos baseados nos IDs
    static void inicializarFilaComJogos(Fila fila, String[] ids, int tamanho) {
        for (int j = 0; j < tamanho; j++) {
            GameAS game = JogosDigitadosM.pegandoUmGame(ids[j]);
            if (game != null) {
                fila.enfileirar(game);
            }
        }
    ...
    static String capturandoOperacao(String entrada) {
        String operacao = "";
        while (contador < 1 && contador < entrada.length()) {
